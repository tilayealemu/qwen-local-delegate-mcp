# Delegation guidance for qwen-delegate-mcp

> **This file exists as a copy-paste source.** After installing `qwen-delegate-mcp`,
...
You have access to a local Qwen model (via the `qwen-delegate` MCP server) that
...
3. **Verify Qwen's output before applying it.** Qwen is smaller and cheaper than
